...
export default async function BrandingSettingsPage() {
  ...
  const result = await getSettingsByGroup(supabase, "business_settings", "branding");
  if (!result.success) return <p className="text-sm text-red-600">Failed to load: {result.error}</p>;
  return <SettingsGroupPage entries={result.data} groupKey="branding" table="business" />;
}
