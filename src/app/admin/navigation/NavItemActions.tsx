...
export function NavItemActionButtons({ item }: { item: NavItem }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <button type="button" onClick={toggleVisibility} className="rounded p-1 text-[#94a3b8] hover:bg-[#f1f5f9] hover:text-[#0f172a]" title={item.visible ? "Hide" : "Show"}>
        {item.visible ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}
      </button>
      <button type="button" onClick={handleDelete} className="rounded p-1 text-[#94a3b8] hover:bg-red-50 hover:text-red-600" title="Delete">
